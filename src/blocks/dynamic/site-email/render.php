...
function render_site_email($block_attributes, $content) { ?>
    <?php $className = isset($block_attributes["className"]) ? $block_attributes["className"] : ''; ?>
    <?php $contact = array(
    ...
    <?php ob_start(); ?>
    <a class="site-contact site-contact--email <?php echo $className ?> is-layout-flex is-content-alignment-center has-small-gap" href="mailto:<?php echo $contact['email'] ?>" target="_blank" rel="noopener noreferrer" title="<?php _e($contact['title'], 'kotisivu-block-theme'); ?>" aria-label="<?php _e($contact['aria-label'], 'kotisivu-block-theme'); ?>" data-track="true" data-type="email" data-label="Email">
