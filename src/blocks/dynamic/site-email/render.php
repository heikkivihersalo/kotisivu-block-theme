...
function render_site_email($block_attributes, $content) { ?>
    ...
    <?php ob_start(); ?>
    <a class="site-contact site-contact--email <?php echo $className ?> is-layout-flex is-content-alignment-center has-small-gap" href="mailto:<?php echo $contact['email'] ?>" target="_blank" rel="noopener noreferrer" aria-label="<?php echo __('Send email to', 'kotisivu-block-theme') . ' ' . $contact['email'] ?>" data-track="true" data-type="email" data-label="Email">
