 ...
 * This function searches for block.json files in the provided directories
 * and returns an array of BlockInfo objects for each discovered block.
 * @param blockFolders - An array of folder patterns to search for blocks
...
export function discoverBlocks(
	...
	if (!Array.isArray(blockFolders) || blockFolders.length === 0) {
		console.warn('Warning: No block folders specified for discovery');
		return [];
	}

	if (!pwd || typeof pwd !== 'string') {
		console.warn(
			'Warning: Invalid working directory provided for block discovery'
		);
		return [];
	}
	...
	for (const folderPattern of blockFolders) {
		if (!folderPattern || typeof folderPattern !== 'string') {
			errors.push(`Invalid folder pattern: ${folderPattern}`);
			continue;
		}

		const fullPath = validateAndNormalizePath(folderPattern, pwd);

		if (!fullPath) {
			errors.push(
				`Could not resolve path for folder pattern: ${folderPattern}`
			);
			continue;
		}
		...
		try {
			...
			if (stat.isDirectory()) {
				const foundBlocks = findBlocksRecursively(fullPath, pwd, 0);
				blocks.push(...foundBlocks);
			} else {
				errors.push(`Path is not a directory: ${folderPattern}`);
			}
		} catch (error) {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			errors.push(
				`Could not access folder ${folderPattern}: ${errorMessage}`
			);
		}
	}
	...
	if (errors.length > 0) {
		console.warn('Block discovery completed with errors:', errors);
	}
