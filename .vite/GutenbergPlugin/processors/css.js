	...
	Object.keys(bundle).forEach((fileName) => {
		...
		if (fileName.endsWith('.css') && chunk.type === 'asset') {
			...
			if (blockPathMatch) {
				...
				const blockSpecificCssPath = resolve(outputDir, `${blockPath}/index.css`);
				...
				try {
					...
					console.warn(`⚠️  Failed to create ${blockPath}/index.css:`, error.message);
				}
			}
		}
	});
...
async function cleanupUnwantedCSSFiles(editorAssetsDir) {
	...
	try {
		...
		const unwantedCssFiles = files.filter(file => 
			file.endsWith('.css') && file.match(/^index\d*\.css$/)
		);
